...
public static class KeyViewerUtils {
    ...
    public static string KeyName(KeyConfig config) => config.DummyName ?? config.Code.ToString();
    ...
    public static void ApplyColorLayout(TextMeshProUGUI text, GColor color) => text.colorGradient = color.gradientEnabled ? (VertexGradient)color : new VertexGradient(color);
    ...
    public static void ApplyColor(Image image, GColor from, GColor to, EaseConfig easeConfig, bool blurEnabled) {
        ...
        if(blurEnabled) {
            ...
            DOVirtual.Float(0, 1, easeConfig.Duration, f => mat.SetColor(Blur_TintColor, EasedColor(from, to, f))).SetEase(easeConfig.Ease).SetAutoKill(false).SetTarget(image);
        }
    ...
    public static void ApplyVectorConfig(RectTransform rt, VectorConfig vConfig, bool pressed, float heightOffset, bool fixScale, Vector2 sizeDelta, bool scaleSizeDelta = true) => ApplyVectorConfig(rt, vConfig, pressed, new Vector2(0, heightOffset), fixScale, sizeDelta, scaleSizeDelta);
    ...
    public static Vector3 FixedScale(Vector3 parentScale, Vector3 fixedScale) => new(fixedScale.x / parentScale.x, fixedScale.y / parentScale.y, fixedScale.z == 0 ? 0 : fixedScale.z / parentScale.z);
    public static Vector2 GetPivot(Pivot pivot) {
        return pivot switch {
            Pivot.TopLeft => new Vector2(0, 1),
            Pivot.TopCenter => new Vector2(0.5f, 1),
            Pivot.TopRight => new Vector2(1, 1),
            Pivot.MiddleLeft => new Vector2(0, 0.5f),
            Pivot.MiddleCenter => new Vector2(0.5f, 0.5f),
            Pivot.MiddleRight => new Vector2(1, 0.5f),
            Pivot.BottomLeft => new Vector2(0, 0),
            Pivot.BottomCenter => new Vector2(0.5f, 0),
    ...
    public static Vector2 AdjustRainPosition(Direction dir, Vector2 position, Vector2 offset) {
        return dir switch {
            Direction.Up or Direction.Left => position.WithRelativeX(offset.x).WithRelativeY(-offset.y),
            Direction.Down or Direction.Right => position.WithRelativeX(-offset.x).WithRelativeY(-offset.y),
            _ => position,
        };
    }
    ...
    public static void MakeBar(Profile profile, List<KeyConfig> keys) {
        ...
        for(int i = 0; i < keys.Count; i++) {
            ...
            if(config.Rain.Direction is Direction.Up or Direction.Down) {
    ...
    public static void SetMultiple<T>(T original, T originalCopy, IEnumerable<T> instances, IEnumerable<T> instanceCopys, string fieldName, System.Func<object, System.Type, bool> relative) where T : IModel, ICopyable<T> => SetMultiple(original, originalCopy, instances, instanceCopys, fieldName, (instance, rRef, t) => relative(instance, t), null);
    public static void SetMultiple<T>(T original, T originalCopy, IEnumerable<T> instances, IEnumerable<T> instanceCopys, string fieldName, System.Func<object, object, System.Type, bool> relative, List<object> relativeIterationRef) where T : IModel, ICopyable<T> {
        ...
        for(int i = 0; i < insts.Count; i++) {
            ...
            if(setRelative && relative(insts[i], relativeIterationRef?[i], valType)) {
    ...
    public static object RelativeOperation(object fromCriterion, object obj, object toCriterion) {
        ...
        return obj is ushort
            ...
            : obj is long
            ? (long)toCriterion + ((long)obj - (long)fromCriterion)
            : obj is ulong
            ? (ulong)toCriterion + ((ulong)obj - (ulong)fromCriterion)
            : obj is double
            ? (double)toCriterion + ((double)obj - (double)fromCriterion)
            : obj is decimal ? (decimal)toCriterion + ((decimal)obj - (decimal)fromCriterion) : obj;
    }
