...
public class Key : MonoBehaviour {
    ...
    public void IgnoreColorUpdate(Element e) => colorUpdateIgnores[(int)e]++;
    ...
    private Vector2 GetMaskPosition(Direction dir) {
        ...
        return dir switch {
            Direction.Up => new Vector2(vec.x + offset.x, vec.y + ((y / 2) - softness) + spacing + offset.y),
            Direction.Down => new Vector2(vec.x + offset.x, vec.y - ((y / 2) - softness) - spacing + offset.y),
            Direction.Left => new Vector2(vec.x + offset.x - ((x / 2) - softness) - spacing, vec.y + offset.y),
            Direction.Right => new Vector2(vec.x + offset.x + ((x / 2) - softness) + spacing, vec.y + offset.y),
            _ => Vector2.zero,
        };
    }
    private Vector2 GetMaskPositionWithoutOffset(Direction dir) {
        ...
        float spacing = Manager.profile.KeySpacing;
        return dir switch {
            Direction.Up => new Vector2(vec.x, vec.y + ((y / 2) - softness) + spacing),
            Direction.Down => new Vector2(vec.x, vec.y - ((y / 2) - softness) - spacing),
            Direction.Left => new Vector2(vec.x - ((x / 2) - softness) - spacing, vec.y),
            Direction.Right => new Vector2(vec.x + ((x / 2) - softness) + spacing, vec.y),
            _ => Vector2.zero,
        };
    }
    private Vector2Int GetSoftness(Direction dir) {
        return dir switch {
            Direction.Up or Direction.Down => new Vector2Int(0, Config.Rain.Softness.Get(Pressed)),
            Direction.Left or Direction.Right => new Vector2Int(Config.Rain.Softness.Get(Pressed), 0),
            _ => Vector2Int.zero,
        };
    }
    #endregion
