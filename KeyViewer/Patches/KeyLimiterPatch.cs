...
public static class KeyLimiterPatch {
    ...
    private static int GetPressedCounts(MainStateCount stateCount, bool async, ButtonState bs) {
        ...
        int count = async
            ? stateCount.keys.Where(k => activeKeys.Contains(AsyncInputCompat.Convert(((AsyncKeyCode)k.value).label))).Count()
            : stateCount.keys.Where(k => activeKeys.Contains((KeyCode)k.value)).Count();

        if(Main.IsWindows) {
            foreach(var mapping in WinInput.GetMappings().Where(k => activeKeys.Contains(k))) {
                if(bs == ButtonState.IsDown && WinInput.GetState(mapping)) {
                    count++;
                }

                if(bs == ButtonState.WentDown && WinInput.IsDown(mapping)) {
                    count++;
                }

                if(bs == ButtonState.WentUp && WinInput.IsUp(mapping)) {
                    count++;
                }
            }
        }
